            ...
            try {
                ...
                if (arrParam){
                    ...
                    arrSearchResults.forEach((data, index) => {
                        ...
                        let objOTData = getTimeBill(empId, arrTaskData, empLaborCost , 'OT')
                        data.custpage_epia0f11_ot_hours_col_09 = objOTData.intHours ? objOTData.intHours : 0
                        data.custpage_epia0f11_ot_earn_10 = objOTData.intAmount ? objOTData.intAmount : 0
                        ...
                        data.custpage_epia0f11_hour_code_exp_col_23 = objEXPData.intHours ? 'EXP' : null
                        data.custpage_epia0f11_earn_code_exp_col_25 = objEXPData.intHours ? 'EXP' : null
                        data.custpage_epia0f11_hour_amt_exp_col_24 = objEXPData.intHours ? objEXPData.intHours : null
...
c
        ...
        const getTimeBill = (empId, arrTaskData, empLaborCost, chargeCode) => {
            ...
            const arrFilteredTask = arrTaskData.filter(item => {
                if (chargeCode === 'S15') {
                    return item.empId === empId && item.stat === true;
                } else if (chargeCode === 'OT') {
                    return item.empId === empId && item.ot === true;
                } else {
                    return item.empId === empId && item.chargeCode.includes(chargeCode);
                }
            });
            ...
            try {
                ...
                let objSavedSearch = search.create({
                    ...
                    filters: [
                        ...
                        'AND',
                        ['employeetype', 'anyof', '15', '18'], // Full Time Employee or Part Time Employee
                        'AND',
                        ...
                        if (pageData.length > 0) {
                            for (let pageResultIndex = 0; pageResultIndex < pageData.length; pageResultIndex++) {
                                ...
                                pageColumns.forEach(function (result) {
                                    ...
                                    if(resultLabel == 'custrecord_epia0f11_employee'){
            ...
            try {
                let objSearch = search.create({
                    ...
                    columns: [
                        ...
                        search.createColumn({ name: 'custcol_time_track_over_time', join: 'time' }),
                        search.createColumn({ name: 'custcol_time_track_statutory_work', join: 'time' }),
                        ...
                        if (pageData.length > 0) {
                            for (var pageResultIndex = 0; pageResultIndex < pageData.length; pageResultIndex++) {
                                arrTaskId.push({
                                    ...
                                    ot: pageData[pageResultIndex].getValue({ name: 'custcol_time_track_over_time', join: 'time' }),
                                    stat: pageData[pageResultIndex].getValue({ name: 'custcol_time_track_statutory_work', join: 'time' })
                                });
                            }
                        }
                    }
            ...
            try {
                ...
                let objSavedSearch = search.create({
                    ...
                    filters: [
                        ...
                        'AND',
                        ['projecttask.custevent_chargecode', 'doesnotcontain', 'STAT'],
                        'AND',
                        ['custcol_time_track_over_time', 'is', 'F'],
                        'AND',
                        ['custcol_time_track_statutory_work', 'is', 'F'],
                    ],
                    columns: [
